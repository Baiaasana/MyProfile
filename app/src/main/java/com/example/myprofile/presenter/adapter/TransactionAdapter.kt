        ...
        fun bind() {
            ...
            if(transaction.id == 1){
                binding.apply {
                    ...
                    tvSubtitle.text = transaction.date.let { getData(it!!.toLong(), "yyyy.MM.dd HH:mm").toString() }
                    tvAmount.text = transaction.amunt.toString()
//                    tvDate.text = transaction.date.let { getData(it!!.toLong(), "yyyy.MM.dd HH:mm").toString() }
                    tvCurrency.text = transaction.currency.toString()
                }
            }else{
                val postTransaction = getItem(bindingAdapterPosition - 1)!!
                if (transaction.date == postTransaction.date) {
                    binding.apply {
                        tvTitle.text = transaction.title.toString()

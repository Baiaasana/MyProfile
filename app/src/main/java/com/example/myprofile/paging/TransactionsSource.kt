...
class TransactionsSource @Inject constructor(private val repository: TransactionsRepositoryImpl) :
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TransactionsUI.TransactionUI> {
        return try{
            ...
            val response = repository.getTransactions(nextPage)

            LoadResult.Page(
                data = response?.items!!.map { it.toDomain().toPresenter() },
